...
resource "aws_autoscaling_group" "default" {
  ...
  launch_configuration = var.create_lc ? element(concat(aws_launch_configuration.default.*.name, [""]), 0) : var.launch_configuration
...
resource "random_pet" "asg_name" {
  ...
  keepers = {
    ...
    lc_name = var.create_lc ? element(concat(aws_launch_configuration.default.*.name, [""]), 0) : var.launch_configuration
  }
}
